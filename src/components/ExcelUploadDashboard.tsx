...
const ExcelUploadDashboard = <T,>({ onData, setFileName }: Props<T>) => {
  ...
  const handleFileChange = (file: File | null) => {
    if (file) {
      ...
      reader.onload = (evt) => {
        ...
        const jsonData: T[] = XLSX.utils.sheet_to_json(ws, {
          raw: false,
          dateNF: "yyyy-mm-dd",
          defval: null,
        });
  ...
  const uploadExcelFile = async (file: File) => {
    ...
    try {
      const res = await axios.post(`${BASE_URL}/api/upload-excel`, formData, {
        headers: {
