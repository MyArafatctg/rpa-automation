...
import FcrSettingForm, { type FcrSettingSettingType } from "./FcrSettingForm";
...
const statusStyles: Record<FcrData["Status"], string> = {
...
const FcrSubReport = ({
  ...
  const [rows, setRows] = useState<FcrData[]>(data);
  ...
  function convertToFcrData(rawData: any[]): FcrData[] {
    return rawData.map((item) => ({
      ...
      FCR_Template: item.FCR_Template,
      Save_As: item.Save_As ?? "",
      Summary_field_description: item.Summary_field_description ?? "",
  ...
  const runBot = async (settingData: FcrSettingSettingType) => {
    ...
    formData.append("botfile", "FcrSubmission");
    formData.append("folder", "D:\\RPA\\logs");
    formData.append("excel_json", JSON.stringify(data));
        ...
        prevRows.map((row) => {
          ...
          if (
            ...
            return {
              ...row,
              Status:
                payload.data?.Status || payload.bot_status || "Processing",
              Err_Description:
                payload.data?.Err_Description ?? row.Err_Description,
              Elapsed_Time:
                payload.data?.Elapsed_Time != null
  ...
  const updatedDBStatus = async (payload: FcrStatusUpdatedPayload) => {
    try {
      const apiUrl = `${BACKEND_URL}/fcr/updateStatus`;
  ...
  const handleDelete = async (id: string, status: string) => {
    ...
    try {
      await axiosInstance.delete(`${BACKEND_URL}/fcr/delete/${id}`);
      ...
      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Booking confirmation number ${id} deleted successfully.`,
        timer: 1000,
      ...
      <div className="overflow-x-auto max-h-[520px] rounded-lg border border-gray-400 shadow-md">
        <table className="min-w-full divide-y divide-gray-400">
          ...
          <tbody className="divide-y divide-gray-400">
            {rows.map((row, index) => (
              <tr key={index} className="hover:transition-all duration-200">
                ...
                <td className="px-6 py-4 text-sm text-black">{row.Save_As}</td>
                ...
                <td className="px-6 py-4 text-sm text-center whitespace-nowrap">
                  <button
                    onClick={() =>
                      handleDelete(row.Booking_confirmation_number, row.Status)
                    }
