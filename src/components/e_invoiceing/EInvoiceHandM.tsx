...
const EInvoiceHandM = ({
  ...
  const runBot = async (settingData: InvoiceSettingData) => {
    ...
    formData.append("botfile", "EInvoiceHnM");
    ...
    try {
      const response = await axios.post(
        `${BASE_URL}/api/start-run-einvoice`, //start-run-json
        formData,
        {
          headers: {
  ...
  const updatedDBStatus = async (payload: InvoiceStatusUpdatePayload) => {
    ...
    try {
      const apiUrl = `${BACKEND_URL}/invoice/updateStatus`;
  ...
  const handleDelete = async (id: string, status: string) => {
    ...
    try {
      await axiosInstance.delete(`${BACKEND_URL}/invoice/delete/${id}`);
